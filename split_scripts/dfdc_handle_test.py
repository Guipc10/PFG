...
if __name__ == "__main__":
    ...
    output_real = os.path.join(output_folder, "0-real")
    if not os.path.exists(output_real):
        os.makedirs(output_real)
    output_fake = os.path.join(output_folder, "1-fake")
    ...
    for key, item in test_json.items():
        ...
        if not os.path.exists(os.path.join(video_new_path, key.split(".mp4")[0])):
